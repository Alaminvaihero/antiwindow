...
# -------------------------------------------------------------------
# Where to look for YARA rules and hash database
SEARCH_PATHS = [
    Path("rules"),                     # rules/ folder (original)
    Path(os.path.expanduser("~/Desktop")),   # your Desktop ("home screen")
    Path(".")                          # script's own directory (loose files)
]
...
# -------------------------------------------------------------------
# Load hash database from all possible locations (SHA256 + MD5)
# -------------------------------------------------------------------
def load_hash_database():
    """Search for malware_hashes.txt in multiple locations and merge all found hashes."""
    sha256_set = set()
    md5_set = set()
    found_any = False
...
def compile_yara_rules():
    """Collect .yar/.yara files from all search paths and compile them together."""
    rule_files = []
    for search_path in SEARCH_PATHS:
    ...
    # Remove duplicates (if same file appears in two paths)
...
# -------------------------------------------------------------------
# Entry point – now defaults to FULL SYSTEM SCAN (C:\ on Windows)
# -------------------------------------------------------------------
def main():
    ...
    if len(sys.argv) > 1:
        ...
        # Default: full system scan on Windows (system drive, usually C:)
        if sys.platform == "win32":
            system_drive = os.environ.get('SystemDrive', 'C:')
            scan_root = Path(f"{system_drive}\\")
            console.print(f"[bold cyan]No directory specified – scanning entire system drive: {scan_root}[/]")
            console.print("[yellow]Note: Full system scan may take a very long time. Press Ctrl+C to stop.[/]")
        else:
            # Fallback for non-Windows (just user home)
            scan_root = Path.home()
